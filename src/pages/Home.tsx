...
// Static projects removed in favor of Supabase fetch
...
export default function Home() {
  ...
  const invertValue = useTransform(scrollYProgress, [0, 0.2], [0, 1]);
  ...
  const secondaryContentOpacity = useTransform(scrollYProgress, [0.05, 0.15], [1, 0]);
  const backgroundElementsOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]);
  
  // UNICO Text Zoom logic - MUST be 1.0 at start (0)
  const unicoScale = useTransform(scrollYProgress, [0.2, 0.45], [1, 80]);
  const unicoOpacity = useTransform(scrollYProgress, [0.4, 0.5], [1, 0]);
  
  // NEW: Transition to BLACK instead of white
  const solidColorOpacity = useTransform(scrollYProgress, [0.4, 0.5], [0, 1]);
      ...
      <section id="portfolio" className="relative z-20 w-full min-h-screen bg-black flex flex-col items-start justify-start text-white py-24 pb-12">
        ...
        >
          {featuredProjects.map((project) => {
            const isVideo = project.cover_image_url && (project.cover_image_url.endsWith('.mp4') || project.cover_image_url.endsWith('.webm'));
            ...
            return (
              <motion.a 
                ...
                variants={blurFadeIn}
                className="relative block w-full rounded-[10px] overflow-hidden aspect-video group cursor-pointer transition-transform duration-500 hover:scale-[1.02] shadow-sm hover:shadow-xl bg-[#141414]"
              >
                {isVideo ? (
                  <video 
                ...
                <div className="absolute top-4 left-4 md:top-6 md:left-6 z-20 flex flex-col gap-3">
                  <h5 
                    className="text-white text-lg sm:text-xl md:text-2xl tracking-wide drop-shadow-md uppercase"
                    style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}
                  >
                    {project.title}
                  </h5>
                  <div className="flex flex-wrap gap-2">
                    <div className="rounded-[5px] px-3 py-1.5 backdrop-blur-[15px]" style={{ backgroundColor: 'rgba(240, 240, 240, 0.25)' }}>
                      <p className="text-[#f4f2ee] text-[9px] sm:text-[10px] md:text-xs font-medium tracking-wide drop-shadow-sm uppercase">
                        {project.category}
                      </p>
                    </div>
                  </div>
...
const Tab = ({ children, href, setPosition }) => {
  ...
  const handleClick = (e) => {
    if (href.startsWith('/')) {
      // Normal browser navigation to route
